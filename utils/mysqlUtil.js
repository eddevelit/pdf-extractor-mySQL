...
const obtenerId = async (campo, valor, tabla) => {
    try {
        console.log(`buscando id de tabla: ${tabla} con los datos => campo: ${campo}, valor: ${valor} `)
        const [result] = await pool.query(`SELECT id FROM ${tabla} WHERE ${campo} = '${valor}'`);
        console.log(`id de ${tabla} obtenido: ${result.id}`)
        return result.id;
    } catch (error) {
        throw error;
    }
...
const mysqlSelect = async (table, fields, where, extraParams) => {
    try {
        ...
        console.log(`Realizando consulta => ${selectQuery} `)
        const [result] = await pool.query(selectQuery);
        console.log(`Consulta realizada, resultado: ${JSON.stringify(result)}`)
        return result;
    } catch (error) {
        throw error;
    }
...
const mySQLInsert = async (table, fields, values) => {
    try {
        ...
        console.log(`Realizando insercion => ${insertQuery} `)
        const insertResult = await pool.query(insertQuery);
        return `Insercion a tabla: ${table}, id: ${insertResult.insertId}`;
    } catch (error) {
        throw error;
    }
}
